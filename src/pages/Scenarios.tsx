...
export default function Scenarios() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-8 animate-slide-up">
      ...
      {/* VOICE MODE (hidden for interview) */}
      {!isInterviewMode && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button
              ...
              className={`rounded-lg border p-4 text-left transition-all relative ${!profile?.is_pro ? "border-border bg-card opacity-50 cursor-not-allowed" : voiceMode ? "border-primary bg-primary/10 card-glow" : "border-border bg-card hover:border-primary/30 card-glow-hover"}`}
            >
              {!profile?.is_pro && (
      ...
      <button
        ...
        className={`w-full rounded-lg py-4 text-lg font-black uppercase tracking-wider transition-all ${canStart && canSimulate ? "gradient-primary text-primary-foreground hover:opacity-90 card-glow" : "bg-secondary text-muted-foreground cursor-not-allowed"}`}
      >
        <span className="flex items-center justify-center gap-2">
          <Zap className="h-5 w-5" />
          {!canSimulate ? "No simulations left — Go Pro"
            : canStart
            ? isInterviewMode ? "Start Interview"
              : simulationMode === "meeting-setter" ? "Enter the Arena — Book the Meeting"
              : "Enter the Arena"
            : isInterviewMode ? "Select difficulty to begin" : "Select all options to begin"}
        </span>
      </button>
